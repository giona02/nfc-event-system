...
// RIMUOVI INSTAGRAM DAL BRACCIALE (opzionale)
app.delete("/bracciali/:id/instagram", async (req, res) => {
  try {
    const { id } = req.params;

    const result = await pool.query(
      "UPDATE bracciali SET instagram = NULL WHERE id = $1 RETURNING *",
      [id]
    );
    ...
    console.error(err);
    res.status(500).json({ error: "Errore rimozione instagram" });
  }
});
